import { getElementRect, normalizeLinkText } from './dom'
...
export const buildTocFromAnchors = (doc: Document): TocEntry[] => {
  ...
  for (const anchor of anchors) {
    ...
    if (upper.match(/^BOOK\s+/)) level = 1
      ...
      upper.match(
        /^(ALL’S|AS\s+YOU|MUCH\s+ADO|A\s+MIDSUMMER|TWELFTH\s+NIGHT|THE\s+TAMING|THE\s+TEMPEST|THE\s+WINTER’S|THE\s+TWO\s+NOBLE)\b/,
      )
    )
      level = 1
