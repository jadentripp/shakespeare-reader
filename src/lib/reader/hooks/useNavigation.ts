...
import { resolveElementFromCfi, getOffsetLeftForRect } from "../navigation";
import { syncPageMetricsToRoot, type PageMetricsResult } from "../pagination";
...
export function useNavigation(options: UseNavigationOptions): UseNavigationResult {
  ...
  const jumpToElement = useCallback((element: HTMLElement): JumpResult => {
  ...
  }, [getScrollRoot, getPageMetrics, columns, margin, pageLockRef, isNavigatingRef, updatePagination]);

  const scrollToCfi = useCallback((cfi: string): boolean => {
    const doc = getDoc();
    const root = getScrollRoot();
