...
import type { BookChatThread, BookMessage, ChatMessage, ChatResult, OpenAiKeyStatus } from './types'

export async function listBookMessages(
  bookId: number,
...
export async function setThreadLastCfi(params: { threadId: number; cfi: string }): Promise<void> {
...
export async function getThreadMaxCitationIndex(
  bookId: number,
  threadId: number | null,
): Promise<number> {
