...
import { afterEach, beforeEach, describe, expect, it, mock, type spyOn } from 'bun:test'
import { CATALOG_BY_KEY, CATALOG_GROUPS, type CatalogEntry } from '../lib/gutenberg'

const MIN_RESULTS_PER_CATEGORY = 4

// Extract all category entries (not collections)
const categoryEntries: CatalogEntry[] = CATALOG_GROUPS.flatMap((group) => group.items).filter(
  (item) => item.kind === 'category',
)

// Extract all collection entries
const collectionEntries: CatalogEntry[] = CATALOG_GROUPS.flatMap((group) => group.items).filter(
  (item) => item.kind === 'collection' || item.kind === 'all',
)
