...
describe('useTTS auto-advance', () => {
  ...
  beforeEach(() => {
    ...
    spies.push(spyOn(audioPlayer, 'getDuration').mockReturnValue(100))
    spies.push(spyOn(audioPlayer, 'getCurrentTime').mockReturnValue(100))
  ...
  it('triggers onPageTurnNeeded when audio ends naturally and autoNext is true', async () => {
    ...
    await act(async () => {
      ...
      if (subscribeCallback) subscribeCallback('playing')
    })

    await act(async () => {
      mockStateValue = 'idle'
      if (subscribeCallback) subscribeCallback('idle')
    })
