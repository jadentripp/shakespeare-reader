...
function detectGroups(entries: TocEntry[]): TocGroup[] {
  ...
  let groupIdx = 0
  entries.forEach((entry, index) => {
    while (groupIdx < groupsToInsert.length) {
      const g = groupsToInsert[groupIdx]
      if (!g || g.firstIndex !== index) break
      result.push({
        type: 'group',
        label: g.label,
        entries: g.entries,
    ...
    >
      ...
      <span
        className={cn(
          ...
          isActive ? 'text-[#E02E2E]' : 'text-foreground group-hover:text-foreground',
        )}
      >
        {entry.text}
      </span>
...
export default function TocPanel({ entries, currentEntryId, onNavigate }: TocPanelProps) {
