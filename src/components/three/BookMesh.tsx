...
interface BookMeshProps {
  ...
  onDragEnd?: ((dropped: boolean) => void) | undefined
  isDragging?: boolean | undefined
  dragOffset?: [number, number, number] | undefined
  initialOffset?: [number, number, number] | undefined
  author?: string | undefined
  downloadCount?: number | undefined
  // Arc animation: book flies from current position to destination
  flyingTo?: [number, number, number] | null | undefined
  onFlyComplete?: (() => void) | undefined
}
  ...
  useFrame((state) => {
    ...
    if (outerGroupRef.current) {
      if (isFlying && flyingTo && flyCurve.current) {
        ...
        const easeT = isTrashDrop ? t * t : t < 0.5 ? 2 * t * t : 1 - (-2 * t + 2) ** 2 / 2
        ...
        <group ref={groupRef}>
          {(hovered || isDragging) && (
            <pointLight
              ...
              color={isDragging ? '#22d3ee' : colors?.accent}
              intensity={isDragging ? 10 : 5}
              distance={isDragging ? 4 : 2}
            />
          )}
          ...
          <mesh
            ...
            onPointerOver={(e) => {
              ...
              document.body.style.cursor =
                draggable && (status === 'remote' || status === 'local') ? 'grab' : 'pointer'
            }}
            onPointerOut={(e) => {
              e.stopPropagation()
              setHovered(false)
          ...
          <mesh position={[-width / 2, 0, 0]} rotation={[0, 0, 0]}>
            <cylinderGeometry
              args={[depth / 2, depth / 2, height, 12, 1, false, Math.PI / 2, Math.PI]}
            />
            <meshStandardMaterial
              color={colors?.spine}
              ...
              <div className="pointer-events-none whitespace-nowrap px-4 py-2.5 bg-stone-950/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl shadow-black/80 flex flex-col items-center gap-0.5 animate-in fade-in zoom-in-95 duration-200">
                ...
                {status === 'remote' && downloadCount !== undefined && (
                  <div className="mt-1 flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-white/5 border border-white/5">
                    <div
                      className={`w-1 h-1 rounded-full ${isHype ? 'bg-amber-400 animate-pulse' : 'bg-white/30'}`}
                    />
