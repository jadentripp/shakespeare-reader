...
const ReadingDesk: React.FC<ReadingDeskProps> = ({ position = [0, 0, 0] }) => {
  ...
  return (
    <group position={position}>
      ...
      <Cylinder
        args={[0.04, 0.04, 0.08]}
        position={[0, height - 0.2, depth / 2 + 0.05]}
      ...
      {/* Side supports */}
      <Box
        args={[0.1, 0.3, depth - 0.4]}
        position={[-width / 2 + 0.15, height - 0.25, 0]}
        castShadow
      >
      ...
      <pointLight position={[0, height - 0.5, 0]} intensity={5} distance={4} color="#FFE4CC" />
    </group>
  )
}
