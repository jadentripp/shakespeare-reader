import { Download, Check, Loader2, BookOpen, Calendar, Users, Tag, Library, Globe, ExternalLink } from "lucide-react";
...
export function BookDetailModal({
    ...
    const authorDates = firstAuthor?.birth_year || firstAuthor?.death_year
        ? `${firstAuthor.birth_year ?? "?"} – ${firstAuthor.death_year ?? "?"}`
        : null;

    // Clean up bookshelves (remove "Category: " prefix)
    const cleanBookshelves = book.bookshelves?.map(b =>
        b.replace(/^Category:\s*/i, "")
    ) ?? [];
