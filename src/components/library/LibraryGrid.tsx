...
export function LibrarySkeleton({ count = 4, variant = "grid" }: { count?: number; variant?: "grid" | "list" | "minimal" }) {
    ...
    return (
      <div className="flex flex-col gap-4">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="flex gap-4 border-2 border-black bg-background p-4 animate-pulse dark:border-white">
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="border-2 border-black bg-background overflow-hidden animate-pulse dark:border-white">
