...
## Phase 2: Fix 3D Component Tests
This phase addresses the rendering issues in React Three Fiber tests.

- [ ] Task: Fix `ReadingRoom.test.tsx`
    - [ ] Analyze `src/components/three/ReadingRoom.tsx` to verify `data-testid` attributes or mesh naming.
    - [ ] Update test queries to correctly target 3D elements (e.g., using `three-test-renderer` patterns or checking how `bun test` handles R3F).
    - [ ] Verify fix by running `bun test src/tests/ReadingRoom.test.tsx`.
- [ ] Task: Fix `BookMesh.test.tsx`
    - [ ] Analyze `src/components/three/BookMesh.tsx` for similar querying issues.
    - [ ] Update test expectations to match the actual rendered structure.
    - [ ] Verify fix by running `bun test src/tests/BookMesh.test.tsx`.
- [ ] Task: Conductor - User Manual Verification 'Fix 3D Component Tests' (Protocol in workflow.md)
