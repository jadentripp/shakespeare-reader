...
- [ ] Task: Create `src/lib/reader/hooks/useMobiReader.ts` and migrate logic.
    - [ ] Write tests for `useMobiReader`.
    - [ ] Migrate state, refs, and TanStack Query hooks.
- [ ] Task: Extract UI components to `src/components/reader/`.
    - [ ] Implement `ReaderToolbar.tsx`.
    - [ ] Implement `ReaderContent.tsx`.
- [ ] Task: Update `MobiBookPage.tsx` to use new hook and components.
